...
signed short prev = 0;
float firVal[] = {.0201, .2309, .4981 ,.2309 ,.0201};
//float firVal[] = {-.0041, .0077, .0893, .2433, .0893, .0077, -.0041};
float iirVal[] = {.25,.75};
...
//    int j;
//    float sum = 0;
//    int min = (bIndex - filterSize > 0) ? (bIndex - filterSize) : -1;
//    for(j = bIndex; j > min; j--)
//    {
//        sum += buffer[j];
//    }
...
//    for(j = bIndex; j > min; j--)
//    {
//        sum += firVal[firIndex] * buffer[j];
//        firIndex++;
//    }
//    return sum + firVal[firIndex];
//}
...
//                        len = sprintf(dataOut, "\r\n%s\t|%7s\t|%7s\t|%7s\t|%7s\t\r\n","Index","Raw","MAF","IIR","FIR");
                    ...
                    {
                        I2C_read_multiple(SLAVE_ADDR , OUTX_L_XL, data, arrLength);
                        signed short accel_z = (data[5] << 8) | data[4];      
                        shiftBuffer(buffer, accel_z);
                        float raw = accel_z;
                        float maf = MAF(buffer);
                        float iir = iirVal[0]*prev + iirVal[1] * accel_z;
                        prev = accel_z;
                        float fir = FIR(buffer);
